...
bool VulkanInstanceLayersAndExtensions::init(std::unordered_set<std::string> requested_layers, std::unordered_set<std::string> requested_extensions) {
    ...
    std::vector<LayerProperty> all_layers = getInstanceLayers<std::vector<LayerProperty>>(
    ...
    for(const LayerProperty& vk_layer : all_layers) {
        ...
        layer_property_and_ext.extensions = getInstanceExtensions<std::vector<LayerExtension>>(
            layer_property_and_ext.property.layer_name.data(),
            [&available_extension_names](const VkExtensionProperties& ext_prop) {
                LayerExtension ext;
    ...
    std::vector<LayerExtension> empty_layer_ext = getInstanceExtensions<std::vector<LayerExtension>>(
        nullptr,
        [&available_extension_names](const VkExtensionProperties& ext_prop) {
            LayerExtension ext;
            ext.extension_name = std::string(ext_prop.extensionName);
...
const std::unordered_map<VulkanInstanceLayersAndExtensions::LayerName, LayerPropertyExt>& VulkanInstanceLayersAndExtensions::getInstanceLayers() const {
    return m_layers;
}

const std::vector<LayerExtension>& VulkanInstanceLayersAndExtensions::getInstanceLayerExtensions(const VulkanInstanceLayersAndExtensions::LayerName& layer_name) const {
    return m_layers.at(layer_name).extensions;
}

bool VulkanInstanceLayersAndExtensions::isLayerSupported(const VulkanInstanceLayersAndExtensions::LayerName& layer_name) const {
    return m_layers.count(layer_name);
}

bool VulkanInstanceLayersAndExtensions::isExtensionSupported(const VulkanInstanceLayersAndExtensions::ExtensionName& ext_name) const {
