...
bool ImageBufferConfig::init(const std::shared_ptr<VulkanDevice>& device, const std::string& name, const pugi::xml_node& image_buffer_data, const std::shared_ptr<VulkanFormatManager>& format_manager) {
    ...
    for (pugi::xml_node view_node = views_node.first_child(); view_node; view_node = view_node.next_sibling()) {
        ...
        view_info_ptr->image_viwe_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        ...
        for (pugi::xml_node flag_node = view_flags_node.first_child(); flag_node; flag_node = flag_node.next_sibling()) {
            view_info_ptr->image_viwe_info.flags |= getImageViewCreateFlag(flag_node.text().as_string());
        }

        view_info_ptr->image_viwe_info.image = VK_NULL_HANDLE;
        view_info_ptr->image_viwe_info.viewType = getImageViewType(view_node.child("ViewType").text().as_string());
        
        view_info_ptr->format = format_manager->getFormat(view_node.child("FormatName").text().as_string());
        view_info_ptr->image_viwe_info.format = view_info_ptr->format->getVkFormat();

        view_info_ptr->image_viwe_info.components = getComponentMapping(view_node.child("Components"));
        ...
        for (pugi::xml_node flag_node = view_node.child("SubresourceRange").child("AspectFlags").first_child(); flag_node; flag_node = flag_node.next_sibling()) {
            view_info_ptr->image_viwe_info.subresourceRange.aspectMask |= getImageAspectFlag(flag_node.text().as_string());
        }

        view_info_ptr->image_viwe_info.subresourceRange.baseMipLevel = view_node.child("SubresourceRange").child("BaseMipLevel").text().as_uint();
        pugi::xml_node level_node = view_node.child("LevelCount");
        std::string level_str = level_node.text().as_string();
        view_info_ptr->image_viwe_info.subresourceRange.levelCount = view_info_ptr->format->getMipLevels();
        

        view_info_ptr->image_viwe_info.subresourceRange.baseArrayLayer = view_node.child("SubresourceRange").child("BaseArrayLayer").text().as_uint();
        view_info_ptr->image_viwe_info.subresourceRange.layerCount = view_info_ptr->format->getArrayLayers();
