...
bool PipelineConfig::init(const std::shared_ptr<VulkanDevice>& device, const pugi::xml_node& pipeline_data) {
    ...
    pugi::xml_node tessellation_state_node = pipeline_data.child("TessellationState");
    ...
    if(multisample_state_node) {
        bool sample_count_as_device = multisample_state_node.attribute("sample_count_as_device").as_bool();
        if(sample_count_as_device) {
            m_multisample_info.rasterizationSamples = device->getMsaaSamples();
        }
        else {
    ...
    if(depth_stencil_state_node) {
        ...
        pugi::xml_node depth_stencil_flags_node = depth_stencil_state_node.child("Flags");
        ...
        pugi::xml_node front_stencil_op_node = depth_stencil_state_node.child("FrontStencilOpState");
        ...
        pugi::xml_node back_stencil_op_node = depth_stencil_state_node.child("BackStencilOpState");
        ...
        m_depth_stencil_info.minDepthBounds = depth_stencil_state_node.child("MinDepthBounds").text().as_float();
    ...
    if(color_blend_state_node) {
        ...
        if(attachments_node) {
            for (pugi::xml_node attachment_node = attachments_node.first_child(); attachment_node; attachment_node = attachment_node.next_sibling()) {
                ...
                attachment.srcAlphaBlendFactor = getBlendFactor(attachment_node.child("SrcAlphaBlendFactor").text().as_string());
                attachment.dstAlphaBlendFactor = getBlendFactor(attachment_node.child("DstAlphaBlendFactor").text().as_string());
                attachment.alphaBlendOp = getBlendOp(attachment_node.child("AlphaBlendOp").text().as_string());

                pugi::xml_node color_write_masks_node = attachment_node.child("ColorWriteMask");
