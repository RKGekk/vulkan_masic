...
const std::shared_ptr<VulkanShader>& VulkanShadersManager::getShader(const std::string& name) const {
    if(!m_shader_name_map.contains(name)) return VulkanShader::NULL_PTR_SHADER;
...
bool VulkanShadersManager::hasShader(const std::string& name) const {
    return m_shader_name_map.contains(name);
}