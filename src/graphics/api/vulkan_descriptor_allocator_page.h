...
public:
    bool init(std::shared_ptr<VulkanDevice> device, VkDescriptorType type, uint32_t num_descriptors);
	void destroy();

	VkDescriptorType GetHeapType() const;
...
private:
	...
	VkDescriptorPool m_vulkan_descriptor_heap;
	VkDescriptorType m_heap_type;
	VkDescriptorSet m_base_descriptor;
