...
struct BoneInstanceDataType {
    uint32_t parent_joint_idx;
    uint32_t child_joint_idx;
};

std::shared_ptr<SceneNode> MeshNodeGeometryGenerator::GenerateBoneLineInstanced(const std::string& mesh_name, const std::string& skeleton_name, float line_width, const std::shared_ptr<VulkanShadersManager>& shader_manager, const std::shared_ptr<SceneNode>& root_transform) {
    using namespace std::literals;
