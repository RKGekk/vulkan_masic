...
bool ApplicationOptions::Init(const std::string& xml_file_name) {
	...
	if (graphics_node) {
		...
		if (color_space_node) {
			std::string sColor_space(color_space_node.first_child().value());
			if (sColor_space == "srgb_nonlinear"s) ColorSpace = RendererColorSpace::SRGB_NONLINEAR;
			else if (sColor_space == "srgb_linear"s) ColorSpace = RendererColorSpace::SRGB_LINEAR;
		}
	}
