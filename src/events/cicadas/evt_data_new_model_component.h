...
class EvtData_New_Model_Component : public BaseEventData {
    ActorId m_actorId = INVALID_ACTOR_ID;
    ComponentId m_componentId = INVALID_COMPONENT_ID;
    std::weak_ptr<SceneNode> m_pSceneNode;

public:
    inline static const EventTypeId sk_EventType = 0xc188c55e;
    //inline static const std::string sk_EventName = "EvtData_New_Model_Component";

    EvtData_New_Model_Component();
    explicit EvtData_New_Model_Component(ActorId actorId, ComponentId componentId, std::weak_ptr<SceneNode> pSceneNode);
    ...
    friend std::ostream& operator<<(std::ostream& os, const EvtData_New_Model_Component& evt);
};